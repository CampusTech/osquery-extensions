...
// `system_profiler SPHardwareDataType -json`. Returns an empty string if the
// JSON is malformed, empty, or missing the expected field.
//
// This is split out as a pure function over bytes so it can be unit-tested
// without invoking system_profiler.
